...
Demo Project for DevOps Bootcamp by [TechWorld with Nana] ([techworld](https://www.techworld-with-nana.com/):
...
6. Add SSH key
   6.1 Navigate on your local machine to .ssh directory in your home folder
   6.2 Copy content of public ssh key `id_rsa.pub`
   6.3 Click at 'Add SSH Key' button at DigitalOcean
   6.4 Paste and give name to public key
...
12. To secure the server, replace existing sources in the rule by your own IP address
    12.1 Determin your public IP address e.g. at [WhatIsMyIpAddress](https://whatwsmyipaddress.com)
    12.2 Replace sources 'All IPv4' and 'All IPv6' with your public IP
...
24. Since it's a bad practice to work with root user, create another user on the system
    24.1 Create new user with `adduser <username>` and set a password
    24.2 Add new user to sudo group with `usermod -aG sudo <username>` for privilege escalation
    24.3 Switch to new user with `su - <username>`
    24.4 In new users home directory create directory `.ssh` and in that the file `authorized_keys`
    25.5 Copy public SSH key from socal machine to the `authorized_keys` file. With that SSH login to new user from local machine is possible
